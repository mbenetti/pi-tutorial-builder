...
function matchesPattern(fileRelativePath, patterns) {
...
function crawlLocalDirectory(dirPath) {
    const filesList = [];
...
function extractTextFromResponse(response) {
    ...
    if (Array.isArray(response.content)) {
        ...
        const textBlocks = response.content.filter((c) => c && c.type === "text" && typeof c.text === "string");
        if (textBlocks.length > 0) {
            return textBlocks.map((c) => c.text).join("\n");
        }

        // 2. Try finding any block with a .text property (fallback if type isn't "text")
        const anyTextBlocks = response.content.filter((c) => c && typeof c.text === "string");
        if (anyTextBlocks.length > 0) {
            return anyTextBlocks.map((c) => c.text).join("\n");
        }

        // 3. Try finding any block with .thinking (fallback if some model returned thinking as only output)
        const thinkingBlocks = response.content.filter((c) => c && c.type === "thinking" && typeof c.thinking === "string");
        if (thinkingBlocks.length > 0) {
            return thinkingBlocks.map((c) => c.thinking).join("\n");
        }

        // 4. Try joining any string elements in the array
        const stringElements = response.content.filter((c) => typeof c === "string");
...
async function callLlmWithRetry(ctx, systemPrompt, prompt, validator, retries = 3, signal, taskName) {
    ...
    for (let attempt = 1; attempt <= retries; attempt++) {
        try {
            ...
            if (typeof global._piLoader_set === "function") {
                if (taskName) {
                    const attemptSuffix = attempt > 1 ? ` (Attempt ${attempt}/${retries})` : "";
                    global._piLoader_set(`${taskName}${attemptSuffix}`);
                }
                else if (attempt > 1) {
                    global._piLoader_set(`Retrying LLM Call... (Attempt ${attempt}/${retries})`);
                }
            }
...
export default function (pi) {
    pi.registerCommand("tutorial", {
        ...
        handler: async (args, ctx) => {
            ...
            const sourceInput = stripQuotes(rawTokens[0]);
            ...
            const runPipeline = async (loader) => {
...
\`\`\``;
                    const rawAbstractions = await callLlmWithRetry(ctx, identifySystem, identifyPrompt, (resp) => {
                        try {
                            const parsed = extractJsonBlock(resp);
                            return Array.isArray(parsed) && parsed.length > 0 && parsed.every(abs => abs && (abs.name || abs.description || abs.file_indices));
                        }
                        catch {
                            return false;
                        }
                    }, 3, signal, "Step 2/6: Identifying core abstractions");
                    const abstractionsRaw = extractJsonBlock(rawAbstractions);
                    if (!Array.isArray(abstractionsRaw))
                        throw new Error("LLM identifying abstractions did not return a array.");
                    // Validate indices and sanitize files map
                    const abstractions = abstractionsRaw.map((abs) => {
                        const validIndices = (abs.file_indices || []).filter((idx) => {
...
Ensure to output only valid JSON.`;
                    const rawRelationships = await callLlmWithRetry(ctx, relationshipSystem, relationshipPrompt, (resp) => {
                        try {
                            const parsed = extractJsonBlock(resp);
                    ...
                    }, 3, signal, "Step 3/6: Determining abstractions connections");
                    const relationshipsRaw = extractJsonBlock(rawRelationships);
                    const relationships = {
                        ...
                        details: (relationshipsRaw.relationships || [])
                            ...
                            .filter((rel) => !isNaN(rel.from) && rel.from >= 0 && rel.from < abstractions.length &&
...
[2, 0, 1, 3]
\`\`\``;
                    const rawOrder = await callLlmWithRetry(ctx, chapterOrderSystem, chapterOrderPrompt, (resp) => {
                        try {
                            const parsed = extractJsonBlock(resp);
                    ...
                    }, 3, signal, "Step 4/6: Sequence mapping");
                    const orderRaw = extractJsonBlock(rawOrder);
...
Provide ONLY the raw Markdown document contents in **${language}**. Do not include backticks surrounding the whole file.`;
                        try {
                            let chapterDoc = await callLlmWithRetry(ctx, writeSystem, writePrompt, (resp) => resp.length > 100, // length verification
                            3, signal);
                    ...
                    global._piLoader_set = undefined;
